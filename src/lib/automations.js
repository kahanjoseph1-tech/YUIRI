...
async function getById(entity, id) {
  ...
  try {
    ...
    try {
      const all = await base44.entities[entity].list("-created_date", 1000);
...
export async function onAppointmentCompleted(appointment) {
  ...
  try {
    const evals = await base44.entities.Evaluation.list("-created_date", 1000);
  ...
  if (appointment.client_id) {
    await base44.entities.Client.update(appointment.client_id, { status: "Evaluating" });
  }
}
...
export async function onEvaluationCompleted(evaluation) {
  ...
  if (evaluation.client_id) {
    await base44.entities.Client.update(evaluation.client_id, {
...
export async function onPlacementEnrolled(placement) {
  if (placement?.client_id) {
    await base44.entities.Client.update(placement.client_id, { status: "Accepted" });
  }
}
